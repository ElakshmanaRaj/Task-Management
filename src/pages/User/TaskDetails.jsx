...
const InfoBox = ({label, value})=>{
  return <>
  <label className="font-medium text-xs text-slate-500">{label}</label>
...
const TodoChecklist = ({isChecked, text, onChange})=>{
  return (
    <div className="flex items-center gap-3 p-3">
      <input type="checkbox" checked={isChecked} onChange={onChange} className="w-4 h-4 bg-gray-100 accent-blue-600 border-gray-300 rounded-sm outline-none cursor-pointer" />
...
const Attachment = ({link, index, onClick})=>{
  return (
    <div className="flex items-center justify-between bg-gray-50 border border-gray-100 px-3 py2 rounded-md mb-3 mt-2 cursor-pointer" onClick={onClick}>
      <div className="flex-1 flex items-center gap-3">
        <span className="text-xs text-gray-400 mr-2 font-semibold">{index < 9 ? `0${index + 1}`: index + 1}</span>
