...
const TaskListTable = ({ tableData }) => {
    ...
    return (
        <div className='p-0 rounded-lg mt-3 overflow-x-auto'>
                ...
                <tbody>
                    {tableData.map((task) => (
                        <tr key={task._id} className='border-t border-gray-200'>
                            <td className='my-3 mx-4 line-clamp-1 text-[13px] overflow-hidden text-gray-700'>{task.title}</td>
                            <td className='py-4 px-4'>
                                <span className={`${getStatusBadgeColor(task.status)} px-2 py-1 inline-block rounded text-xs`}>
                                    {task.status}
                                </span>
                            </td>
                            <td className='px-4 py-4'>
                                <span className={`${getPriorityBadgeColor(task.priority)} px-2 py-1 inline-block rounded text-xs`}>
                                    {task.priority}
                                </span>
                            </td>
                            <td className='px-4 py-4 line-clamp-1 text-gray-700 text-[13px] text-nowrap hidden md:table-cell'>
